...
namespace Botmoww.AdminCommands
{
    public class AdminCommands : ModuleBase<SocketCommandContext>
    {
        ...
        #region filter
        [Command("filter")]
        public async Task Filter(string filterWord)
        {
            if (filterWord.EndsWith('-') || filterWord.EndsWith('+'))
            { if (filterWord.EndsWith('+'))
                { if (Globals.filterList.Contains(filterWord.TrimEnd('+'))) { await Context.Message.Channel.SendMessageAsync($">>> The word **{filterWord.TrimEnd('+')}** already exists! :red_car:"); }
                    else { Globals.filterList.Add(filterWord.TrimEnd('+')); await Context.Message.Channel.SendMessageAsync($">>> The word **{filterWord.TrimEnd('+')}** was added! :blue_car:"); } }                
